...
def generate_prompt(product_name, product_desc, num, max_length, keywords):
    prompt = f"""
제품 혹은 브랜드를 SNS에 광고하기 위한 문구를 {num}개 생성해 주세요.
반드시 {max_length} 단어 이내로 생성해 주세요.
자극적이고 창의적으로 작성하세요.
명사 위주로 간결하게 작성해 주세요.
이모지를 적절하게 섞어주세요.
...
with st.form("form"):
    col1, col2, col3 = st.columns(3)  # 화면 배열 나누기
    with col1:
        name = st.text_input(
            "제품/브랜드 이름(필수)",
...
if submit:
    if not name:
        st.error("제품/브랜드 이름을 입력해 주세요.")
    elif not desc: #혹은
        st.error("제품 간단 정보를 입력해 주세요.")
    else:
